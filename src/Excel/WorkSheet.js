    ...
    _.extend(Worksheet.prototype, {
        ...
        */
        ...
        toXML: function () {
			...
			var posXY = this._posXY;
			var tblCol = this._tblCol;
			var style = this._style;
			var doc = util.createXmlDoc(util.schemas.spreadsheetml, 'worksheet');
            var worksheet = doc.documentElement;
            var i, l, row,t,tl,start, end;
			var rowPos = [], rowTblPos, rowData = [], posData= [];
			...
			/* Get the data and position to map in Excel */
            var chunkData = _.chunk(data, tblCol);
			var chunkPosData = 	_.chunk(posXY, tblCol);
		
			for (var i=0; i<chunkData.length; i++) {
				rowData[i] = _.zip(chunkData[i][0], chunkData[i][1]);
			}

			for (var i=0; i<chunkPosData.length; i++) {
				posData[i] = [chunkPosData[i][0],chunkPosData[i][1]] ;
			}
		
			// /* Loop - data rows */			
			for(var tblData = 0; tblData< rowData.length ;tblData++) {
				var tblRowData = rowData[tblData];
				/* Loop - table rows */ 
				for(var tblRow =0; tblRow < tblRowData.length;tblRow++) {
					var rowNode = doc.createElement('row');
					if(!_.isUndefined(rowPos[tblData])) {
						rowPos[tblData]++;
					}
					/* Loop - table columns */ 
					for(var tblCol = 0; tblCol < tblRowData[tblRow].length; tblCol++ ) {
						if( _.isUndefined(tblRowData[tblRow][tblCol]) || (!_.isUndefined(tblRowData[tblRow][tblCol][0]) && !_.isNull(tblRowData[tblRow][tblCol][0]) && !_.isUndefined(tblRowData[tblRow][tblCol][0]['IMG']))) { colPos++; continue; }
						var colPos =  posData[tblData][tblCol][0];
						if(_.isUndefined(rowPos[tblData])) {
							rowPos[tblData] = posData[tblData][tblCol][1];
						} 
						rowTblPos = rowPos[tblData];
						if(typeof tblRowData[tblRow][tblCol] == 'undefined') { colPos++; continue; }
						/* Loop - table column values */
						for(var tblCell = 0; tblCell < tblRowData[tblRow][tblCol].length; tblCell++ ) {
							columns[tblCell] = columns[tblCell] || {};
							var cellValue = tblRowData[tblRow][tblCol][tblCell];
							var cell, metadata = cellValue && cellValue.metadata || {};
							var header = cellValue && cellValue.header || {};
							if (cellValue && typeof cellValue === 'object') {
								cellValue = cellValue.value;
							}
							
							var digit = new RegExp(/^[-]*\d*[,.\d]+$/);
							var numHasComma = new RegExp(/,/g);
							var numHasMinusSign = new RegExp(/-/);
							var numHasExponentZero = new RegExp(/[1-9]+/g);
							
							if(digit.test(cellValue)) {
			
								if(numHasComma.test(cellValue)) {
									cellValue = cellValue.replace(/,/g,'');
								}
								
								if(numHasMinusSign.test(cellValue)) {
									
									var exponent = cellValue.split('.');
									if(!numHasExponentZero.test(exponent[1])){
										cellValue = exponent[0];
									}
								}
								metadata.type = 'number';	
							}
							...
							if(tblRow == 0 ) {
								metadata.style =  metadata && metadata.style ? metadata.style : style.title ? style.title : '';
							} else if(digit.test(cellValue) && header == true) {
								metadata.style =  metadata && metadata.style ? metadata.style : style.headerNumber ? style.headerNumber : '';
							} else if(digit.test(cellValue)) {
								metadata.style =  metadata && metadata.style ? metadata.style : style.number ? style.number : '';
							} else if(header == true) {
								metadata.style =  metadata && metadata.style ? metadata.style : style.header ? style.header : '';
							} else {
								metadata.style =  metadata && metadata.style ? metadata.style : style.cell ? style.cell : '';
							}
							...
							} else if (this._rowInstructions[rowTblPos] && this._rowInstructions[rowTblPos].style !== undefined) {
								cell.setAttribute('s', this._rowInstructions[rowTblPos].style);
							}
							cell.setAttribute('r', util.positionToLetterRef(colPos, rowTblPos));
							rowNode.appendChild(cell);
							colPos++;
						}	
						rowNode.setAttribute('r', rowTblPos);
						if (this._rowInstructions[rowTblPos]) {
							var rowInst = this._rowInstructions[rowTblPos];
		...
		setPosXY: function(xy){
			this._posXY.push(xy);
		},
		
		setTblCol: function(value){
			this._tblCol = value;
		},
